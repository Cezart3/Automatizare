    ...
    <div style={{ display: 'flex', minHeight: '100vh', width: '100vw', overflow: 'hidden' }}>
      ...
      <aside className="glass-panel" style={{ width: '280px', margin: '16px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '24px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <h1 style={{ fontSize: '1.25rem', fontWeight: 600, letterSpacing: '-0.025em' }}>AutoConfigurator</h1>
        </div>
        
        <nav style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
          <NavItem icon={<LayoutDashboard />} label="Dashboard" active={activeTab === 'dashboard'} onClick={() => setActiveTab('dashboard')} />
          <NavItem icon={<Mail />} label="Inbox Automat" active={activeTab === 'inbox'} onClick={() => setActiveTab('inbox')} />
          <NavItem icon={<FileText />} label="Comenzi PDF" active={activeTab === 'orders'} onClick={() => setActiveTab('orders')} />
          <NavItem icon={<Users />} label="Clienti" active={activeTab === 'customers'} onClick={() => setActiveTab('customers')} />
        </nav>
        
        <div style={{ padding: '16px', borderTop: '1px solid var(--border-color)' }}>
          <NavItem icon={<Settings />} label="Setari" active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} />
        </div>
      </aside>
      ...
      <main style={{ flex: 1, padding: '16px 32px 32px 16px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '24px', paddingTop: '8px' }}>
          <div>
            <h2 style={{ fontSize: '1.75rem', fontWeight: 600, marginBottom: '4px' }}>Overview</h2>
